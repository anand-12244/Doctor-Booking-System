...
const RelatedDoctors = ({ doctors, currentDocId, speciality, navigation }) => {
  const related = doctors
    .filter((d) => d.speciality === speciality && d._id !== currentDocId)
    .slice(0, 5);
      ...
      >
        {related.map((doc) => (
          <TouchableOpacity
            ...
            style={rel.card}
            onPress={() =>
              navigation.replace("Appointment", { docId: doc._id })
            }
            activeOpacity={0.85}
          >
            ...
            <View style={rel.cardBody}>
              ...
              <Text style={rel.name} numberOfLines={1}>
                {doc.name}
              </Text>
              <Text style={rel.spec} numberOfLines={1}>
                {doc.speciality}
              </Text>
            </View>
          </TouchableOpacity>
...
const AppointmentScreen = ({ route, navigation }) => {
  ...
  const { doctors, backendUrl, token, userData, getDoctors, currencySymbol } =
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      i === 0
        ? curr.setHours(
            curr.getHours() >= 10 ? curr.getHours() + 1 : 10,
            0,
            0,
            0,
          )
  ...
  const bookAppointment = async () => {
    ...
    if (!slotTime)
      return Alert.alert("Select Slot", "Please select a time slot.");
    const day0 = docSlots[slotIndex];
    if (!day0?.length)
      return Alert.alert("No Slots", "No slots available for this day.");
    ...
    try {
      ...
      if (data) {
        // Update doctor's slots_booked in json-server
        ...
        await axios.patch(backendUrl + `/doctors/${docId}`, {
      ...
      >
        ...
        <View style={s.doctorCard}>
          <View style={s.imgContainer}>
            <Image
              source={getDoctorImage(docInfo.imageKey || docInfo._id)}
              style={s.docImage}
              resizeMode="contain"
            />
          </View>
