...
const MyAppointmentsScreen = ({ navigation }) => {
  ...
  const getUserAppointments = async () => {
    try {
      // Using json-server: GET /appointments?userId=token
      const { data } = await axios.get(
        backendUrl + `/appointments?userId=${token}`,
      );
      if (data) {
  ...
  const cancelAppointment = async (appointmentId) => {
    try {
      // Using json-server: DELETE /appointments/:id
  ...
  const renderItem = ({ item }) => (
    <View style={styles.card}>
      <View style={styles.cardTop}>
        <Image
          source={getDoctorImage(item.docData?.imageKey || item.docData?._id)}
      ...
      <View style={styles.btnContainer}>
        ...
        {item.cancelled && (
          <View style={styles.statusBtnCancelled}>
            <Text style={styles.statusBtnCancelledText}>
